...
#        print(state_vector)
        # Qpos = [state_vector[0],state_vector[1]]
        # Qvel = [state_vector[2],state_vector[3]]
        #print(Qpos)
        target = []
        for j in range(self.batch_size):
            vH=0
            Qpos =np.array([state_vector[j][0],state_vector[j][1]])
            Qvel =np.array([state_vector[j][2],state_vector[j][3]])
            self.model.reset()
            self.model.set_state(Qpos,Qvel)
            sim_action=self.choose_action(state_vector[j])
